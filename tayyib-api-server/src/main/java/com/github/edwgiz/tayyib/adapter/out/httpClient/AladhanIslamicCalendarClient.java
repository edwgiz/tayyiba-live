    ...
    public @Nullable GregorianToHijriCalendar gregorianToHijriCalendar(
        ...
        return restClient
                ...
                .exchange((_, clientResponse) -> {
                    ...
                    final var hijri = new LocalDate[responseDto.data().length];
                    final var datePairs = responseDto.data();
                    for (int i = 0; i < datePairs.length; i++) {
                        var datePair = datePairs[i];
                        gregorian[i] = toModel(datePair.gregorian());
                        hijri[i] = toModel(datePair.hijri());
                    }
                    return new GregorianToHijriCalendar(gregorian, hijri);
                });
    }

    private static LocalDate toModel(final GregorianToHijriCalendarResponse.GregorianHijriPair.Date dateDto) {
        return LocalDate.of(
